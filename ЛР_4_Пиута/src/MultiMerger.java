public class MultiMerger extends Thread {
    ...
    public void run() {
        ...
        if ( unsorted.length <= 1 ) {
            ...
            // Пока не превысили максимальное количество потоков, запускаем рекурсивно новые потоки на 2-х новых массивах
            if ( activeCount() < MAX_THREADS ) {
                MultiMerger leftSort = new MultiMerger( left );
                MultiMerger rightSort = new MultiMerger( right );
                leftSort.start();
                rightSort.start();
                // Соединяем, как только потоки дождутся друг друга
                try {
                    leftSort.join();
                    rightSort.join();
